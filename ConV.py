...
def validate(model, loader, crops, fixed_threshold=0.0138, aggregation='mean'):
    """Validate the model.
    
    Args:
        aggregation: 'mean', 'max', or 'min' - how to aggregate distances across crops
    """
    model.cuda()
    model.eval()
    ...
    with torch.no_grad():
        ...
        for img, labels in loader:
            ...
            for i in range(crops * 1):
                original_embedding1 = model(img["source"][i].half().cuda())
                original_embedding1 = original_embedding1 / original_embedding1.norm(dim=-1, keepdim=True)
                original_embedding = original_embedding1
...
def main():
    ...
    parser.add_argument('--aggregation', type=str, default='mean', choices=['mean', 'max', 'min'],
                        help='How to aggregate distances across crops: mean, max, or min')
